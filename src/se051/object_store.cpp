#include <se051/object_store.hpp>
...
bool ObjectStore::Exists(uint32_t id) {
    ...
    SE051_LOG_DEBUG("se: exists id=0x%08x -> %s", static_cast<unsigned>(id),
                   found ? "yes" : "no");
    return found;
}
...
Result<ObjectType> ObjectStore::GetType(uint32_t id) {
    ...
    SE051_LOG_DEBUG("se: type id=0x%08x cipherType=%u -> %d",
...
Status ObjectStore::Erase(uint32_t id) {
    SE051_LOG_DEBUG("se: erase id=0x%08x", static_cast<unsigned>(id));
    KeyObject obj(conn_);
    auto st = obj.Open(id);
    if (!st)
        return st;
    ...
    SE051_LOG_DEBUG("se: erase id=0x%08x OK", static_cast<unsigned>(id));
    return Ok();
}
...
Status ObjectStore::StoreBinary(uint32_t id, const uint8_t *data, size_t data_len,
    ...
    if (sst != kStatus_SSS_Success) {
        SE051_LOG_ERROR("se: %s: sss_key_store_set_key failed (0x%04x)", tag,
...
Status ObjectStore::WriteCert(uint32_t id, const uint8_t *der, size_t der_len) {
    SE051_LOG_DEBUG("se: writeCert id=0x%08x len=%zu", static_cast<unsigned>(id), der_len);
    if (Exists(id)) {
        SE051_LOG_DEBUG("se: writeCert: erasing existing 0x%08x", static_cast<unsigned>(id));
        auto st = Erase(id);
        if (!st) return st;
    }
...
Result<bool> ObjectStore::WriteBinary(uint32_t id, const uint8_t *data, size_t data_len,
                                       bool force) {
    SE051_LOG_DEBUG("se: writeBinary id=0x%08x len=%zu force=%d",
                   static_cast<unsigned>(id), data_len, force);
    if (Exists(id)) {
        if (!force) {
            SE051_LOG_INFO("se: writeBinary: 0x%08x exists (use force to overwrite)",
                          static_cast<unsigned>(id));
            return false;
        }
        auto st = Erase(id);
...
Result<ObjectStore::BlobData> ObjectStore::ReadBinary(uint32_t id) {
    SE051_LOG_DEBUG("se: readBinary id=0x%08x", static_cast<unsigned>(id));
    KeyObject obj(conn_);
    if (auto st = obj.Open(id); !st)
        return SeFail(kReadFailed, "readBinary: object not found");

    BlobData buf;
    buf.resize(SE051_SE_BLOB_CAPACITY);
    size_t len = buf.size();
...
Result<bool> ObjectStore::VerifyBinding(uint32_t key_id, const uint8_t *cert_der,
    ...
    SE051_LOG_DEBUG("se: verifyBinding key=0x%08x result=%s",
